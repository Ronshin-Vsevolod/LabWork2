...
public:
    ...
    void apply(Personage* target) override
    {
        ...
        {
            target->health -= value;
            std::cout << target->name << " gets " << value << " damage from the effect " << name << "!\n";
        }
    }
...
public:
    ...
    void apply(Personage* target) override
    {
        ...
        {
            target->health -= value;
            std::cout << target->name << " gets " << value << " additional damage from the effect " << name << "!\n";
        }
    }
...
TEST(EnemyAITest, BasicBehavior)
{
    ...
    auto blockingEnemy = std::make_shared<Enemy>("Blocking enemy", 10, 10, 2, true);
    ...
    auto blockingEnemyLeft = std::make_shared<Enemy>("Blocking enemy left", 10, 10, 2, true);
...
TEST(EnemyAITest, MilitaryBehavior)
{
    ...
    auto rangedSkill = std::make_shared<ClassicSkill>("Ranged attack", 2, 2, std::vector<int>{2}, 0, 0);

    auto meleeSkill = std::make_shared<ClassicSkill>("Melee attack", 1, 1, std::vector<int>{1}, 0, 0);
    ...
    EXPECT_EQ(enemy->prepareStack[0]->name, "Ranged attack");
...
TEST(LevelSystemTest, LevelSystem)
{
        ...
        WaveInfo("Wave 1", 0, {"Ashigaru", "Twin Tachi"}),
        WaveInfo("Wave 2", 5, {"Ashigaru", "Twin Tachi", "Yari Master"})
        };
...
TEST(LevelSystemTest, EnemyWaves)
{
        ...
        {"Ashigaru", "Twin Tachi"}),
        WaveInfo("Second wave", 5,{"Yari Master"})};
    LevelData testLevel(1, 50, 10, 5, testWaves);
